  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 left-4 z-20">
        <Button variant="ghost" size="sm" className="text-white hover:bg-white/10" onClick={() => setMenuOpen(!menuOpen)}>
          <MoreVertical className="w-6 h-6" />
        </Button>
        {menuOpen && (
          <div className="absolute right-0 mt-1 w-40 bg-gray-900/90 rounded-lg shadow-lg border border-gray-700 py-1">
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      <AnimatePresence>
        {viewerOpen && (
          <FullscreenViewer media={allMedia} initialIndex={viewerIndex} onClose={() => setViewerOpen(false)} onDelete={(media) => { setSelectedMedia(media); setDeleteMediaDialogOpen(true); }} onIndexChange={(index) => setViewerIndex(index)} />
        )}
      </AnimatePresence>
